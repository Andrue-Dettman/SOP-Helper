...
const readyBuild: BuildInventoryResult = {
  ...
  assembly_id: 'asm-kit-a-std',
  assembly_label: 'Kit A — Standard',
  requested_units: 20,
  ready: true,
  components: [
    { part_id: 'prt-1', label: 'Bracket', per_assembly: 2, required: 40, available: 120, shortage: 0 },
  ],
}
...
const notReadyBuild: BuildInventoryResult = {
  ...
  ready: false,
  components: [
    { part_id: 'prt-2', label: 'Side Panel A', per_assembly: 1, required: 20, available: 12, shortage: 8 },
  ],
}
...
const unknownReadyBuild: BuildInventoryResult = {
  ...
  ready: null,
  components: [
    { part_id: 'prt-3', label: 'Fastener Kit', per_assembly: 1, required: 20, available: null, shortage: null },
  ],
}
...
const stockResult: StockInventoryResult = {
  ...
  state: 'ok',
  snapshot: { snapshot_id: 'snap-1', captured_at: '2026-09-14T08:00:00Z' },
  matches: [{ part_id: 'prt-2', label: 'Side Panel A', quantity: 32, unit: 'each' }],
}
...
describe('InventoryResult', () => {
  it('shows a ready banner and the assembly/quantity for a ready build', () => {
    ...
    expect(screen.getByRole('heading', { name: /kit a — standard.*20 units/i })).toBeInTheDocument()
  })
