...
const components: BuildComponent[] = [
  { part_id: 'prt-bracket-01', label: 'Mounting Bracket', per_assembly: 2, required: 40, available: 120, shortage: 0 },
  { part_id: 'prt-panel-a', label: 'Side Panel A', per_assembly: 1, required: 20, available: 12, shortage: 8 },
]
...
describe('ComponentTable', () => {
  it('lists every component with its required, available, and shortage figures', () => {
    ...
    expect(shortRow.getByText('Side Panel A')).toBeInTheDocument()
  ...
  it('renders a null available/shortage value as unknown rather than zero', () => {
    render(
      <ComponentTable
        components={[
          { part_id: 'prt-x', label: 'Unknown Part', per_assembly: 1, required: 5, available: null, shortage: null },
        ]}
      />,
    )
